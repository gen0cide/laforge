// Package tfesx implements a Laforge Builder module for generating terraform configurations that target VSphere ESXi.
package tfesx
...
import (
	...
	"github.com/gen0cide/laforge/builder/tfesx/static"
...
// TerraformESXBuilder implements a laforge builder that packages an environment into
// a terraform configuration targeting GCP with each team isolated into their own VPC.
type TerraformESXBuilder struct {
	sync.RWMutex
...
func (t *TerraformESXBuilder) Get(key string) string {
	t.Lock()
	defer t.Unlock()
