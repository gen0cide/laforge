...
func (fdq *FileDownloadQuery) sqlAll(ctx context.Context) ([]*FileDownload, error) {
	...
	if query := fdq.withFileDownloadToEnvironment; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].environment_environment_to_file_download; fk != nil {
				ids = append(ids, *fk)
...
func (fdq *FileDownloadQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := fdq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, filedownload.ValidColumn)
			}
		}
	}
	return _spec
...
func (fdq *FileDownloadQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	for _, p := range fdq.order {
		p(selector, filedownload.ValidColumn)
	}
	if offset := fdq.offset; offset != nil {
...
func (fdgb *FileDownloadGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range fdgb.fns {
		columns = append(columns, fn(selector, filedownload.ValidColumn))
	}
