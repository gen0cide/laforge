...
type CommitQuery struct {
	...
	// eager-loading edges.
	withCommitToBuild     *BuildQuery
	withCommitToPlanDiffs *PlanDiffQuery
	withFKs               bool
...
// QueryCommitToBuild chains the current query on the "CommitToBuild" edge.
func (cq *CommitQuery) QueryCommitToBuild() *BuildQuery {
...
// QueryCommitToPlanDiffs chains the current query on the "CommitToPlanDiffs" edge.
func (cq *CommitQuery) QueryCommitToPlanDiffs() *PlanDiffQuery {
	query := &PlanDiffQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommitQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (cq *CommitQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (cq *CommitQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (cq *CommitQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (cq *CommitQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (cq *CommitQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CommitQuery) Clone() *CommitQuery {
	...
	return &CommitQuery{
		...
		withCommitToBuild:     cq.withCommitToBuild.Clone(),
		withCommitToPlanDiffs: cq.withCommitToPlanDiffs.Clone(),
...
// WithCommitToBuild tells the query-builder to eager-load the nodes that are connected to
// the "CommitToBuild" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommitQuery) WithCommitToBuild(opts ...func(*BuildQuery)) *CommitQuery {
	query := &BuildQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommitToBuild = query
	return cq
}

// WithCommitToPlanDiffs tells the query-builder to eager-load the nodes that are connected to
// the "CommitToPlanDiffs" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommitQuery) WithCommitToPlanDiffs(opts ...func(*PlanDiffQuery)) *CommitQuery {
	query := &PlanDiffQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommitToPlanDiffs = query
	return cq
}
...
//
//	client.Commit.Query().
//		Select(commit.FieldType).
//		Scan(ctx, &v)
//
func (cq *CommitQuery) Select(field string, fields ...string) *CommitSelect {
...
func (cq *CommitQuery) sqlAll(ctx context.Context) ([]*Commit, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [2]bool{
			cq.withCommitToBuild != nil,
			cq.withCommitToPlanDiffs != nil,
		}
	)
	if cq.withCommitToBuild != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withCommitToBuild; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Commit)
		for i := range nodes {
			if nodes[i].commit_commit_to_build == nil {
				continue
			}
			fk := *nodes[i].commit_commit_to_build
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "commit_commit_to_build" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CommitToBuild = n
			}
		}
	}

	if query := cq.withCommitToPlanDiffs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Commit)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.CommitToPlanDiffs = []*PlanDiff{}
		}
		query.withFKs = true
		query.Where(predicate.PlanDiff(func(s *sql.Selector) {
			s.Where(sql.InValues(commit.CommitToPlanDiffsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.plan_diff_plan_diff_to_commit
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "plan_diff_plan_diff_to_commit" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "plan_diff_plan_diff_to_commit" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CommitToPlanDiffs = append(node.Edges.CommitToPlanDiffs, n)
		}
	}
...
func (cq *CommitQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(commit.Columns...)...).From(t1)
	if cq.sql != nil {
		selector = cq.sql
		selector.Select(selector.Columns(commit.Columns...)...)
	}
...
func (cs *CommitSelect) sqlQuery() sql.Querier {
	selector := cs.sql
